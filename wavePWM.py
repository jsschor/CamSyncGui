...
class PWM:
   ...
   def __init__(self, pi, frequency=1000):
      ...
      self.frequency = [frequency]*self._MAX_GPIO
      self.micros = [1000000.0 / frequency]*self._MAX_GPIO
   ...
   def set_frequency(self, gpio, frequency):
      """
      Sets the PWM frequency in Hertz.

      The change takes affect when the update function is called.
      """
      self.frequency[gpio] = float(frequency)
      self.micros[gpio] = 1000000.0 / self.frequency[gpio]
   ...
   def get_frequency(self,gpio):
      """
      Returns the frequency in Hertz.
      """
      return self.frequency[gpio]
   ...
   def set_pulse_length_in_micros(self, gpio, length):
      ...
      The change takes affect when the update function is called.
      """
      length %= self.micros[gpio]

      self.pL[gpio] = length / self.micros[gpio]
   ...
   def set_pulse_start_in_micros(self, gpio, start):
      ...
      start %= self.micros[gpio]

      self.pS[gpio] = start / self.micros[gpio]
   ...
   def updateWaveChain(self,gpio,freq,startDel,pW,repeats):
      """
      ...
      for g in range(self._MAX_GPIO):
         ...
         if self.used[g]:
            ...
            on = int(self.pS[g] * self.micros[g])
            length = int(self.pL[g] * self.micros[g])

            micros = int(self.micros[g])
      ...
      if not null_wave:
         ...
         if not self.stop:
            ...
            self.wid.append(self.pi.wave_create())
            self.repeats.append(repeats)
            self.widList.extend([255,0])
            self.widList.append(self.wid[-1])
            self.widList.extend([255,1,repeats%256,int(np.floor(repeats/256))])
   
   def updateWaveChainSeconds(self,gpio,freq,startDel,pW,repeats):
      """
      Updates the PWM for each GPIO to reflect the current settings.
      """
      
      self.set_frequency(gpio,freq)
      self.set_pulse_start_and_length_in_micros(gpio, startDel, pW)
      ...
      for g in range(self._MAX_GPIO):
         ...
         if self.used[g]:
            ...
            for i in range(int(1000000.0/micros)):
               pulseList = []
#                if 0:
#                    break
               if length <= 0:
                  pulseList.append(pigpio.pulse(0, 1<<g, micros))
               elif length >= micros:
                  pulseList.append(pigpio.pulse(1<<g, 0, micros))
               else:
#                   print('yaas')
                  off = (on + length) % micros
                  if on < off:
...
#                      print('ayyy')
                     if i>0:
                         pulseList.append(pigpio.pulse(0,0,self.pi.wave_get_micros()))
                     pulseList.extend([
                        pigpio.pulse(   0, 1<<g,           on),
                        pigpio.pulse(1<<g,    0,     off - on),
                        pigpio.pulse(   0, 1<<g, micros - off),
               ...
               self.pi.wave_add_generic(pulseList)
      if not null_wave:
   ...
   def updateWaveChainBipolarSeconds(self,gpio,freq,startDel,pW,ipi,repeats):
      """
      Updates the PWM for each GPIO to reflect the current settings.
      """
               ...
               if length <= 0:
                  ...
                  if on < off:
                     ...
                     pulseList.extend([
                        pigpio.pulse(   0, 1<<g,           on),
                        pigpio.pulse(1<<g,    0,     off - on),
                        pigpio.pulse(   0, 1<<g,     ipi),
                        pigpio.pulse(1<<g,    0,     off - on),
                        pigpio.pulse(   0, 1<<g, micros - (2*(off-on)+ipi)),
                        ])
                  else:
                     if i>0:
                         pulseList.append(pigpio.pulse(0,0,self.pi.wave_get_micros()))
   ...
   def updateWaveChainDelaySeconds(self,gpio,delays):
      """
      Updates the PWM for each GPIO to reflect the current settings.
      """
      null_wave = True
      self.set_frequency(gpio,1)
      self.set_pulse_start_and_length_in_micros(gpio, 0, 1000)
         ...
         if self.used[g]:
            ...
            if length <= 0:
               ...
               if on < off:
                  self.pi.wave_add_generic([
                     pigpio.pulse(   0, 1<<g,           on),
                     pigpio.pulse(0,    1<<g,     off - on),
                     pigpio.pulse(   0, 1<<g, micros - off),
                     ])
               else:
                  self.pi.wave_add_generic([
                     pigpio.pulse(0,    1<<g,         off),
                     pigpio.pulse(   0, 1<<g,    on - off),
                     pigpio.pulse(0, 1<<g, micros - on),
                     ])
      ...
      if not null_wave:
         ...
         if not self.stop:
            ...
            self.widList.extend([255,0])
            self.widList.append(self.wid[-1])
            self.widList.extend([255,1,delays%256,int(np.floor(delays/256))])

   def updateWaveChainStartLoop(self):
      self.widList.extend([255,0])
       
   def updateWaveChainEndLoop(self,repeats):
      self.widList.extend([255,1,repeats%256,int(np.floor(repeats/256))])

   def startWaveChain(self):
      widList = self.widList
      
      self.pi.wave_chain(widList)
      ...
      for wids in self.wid:
         self.pi.wave_delete(wids)
